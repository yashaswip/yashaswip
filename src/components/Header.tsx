  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      ...
      <div className="container mx-auto px-4 py-4">
        ...
        {/* Mobile Navigation */}
        {isMenuOpen && (
          <nav className="md:hidden mt-4 pb-4 bg-slate-900/95 backdrop-blur-md rounded-lg border border-slate-800/50 shadow-lg">
            <div className="flex flex-col space-y-4 p-4">
              {navItems.map((item) => (
                <button
                  key={item.name}
                  onClick={() => scrollToSection(item.href)}
                  className="text-slate-300 hover:text-white transition-colors duration-200 font-medium text-left py-2"
                >
                  {item.name}
                </button>
              ))}
